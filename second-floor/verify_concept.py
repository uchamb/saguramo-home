...
assert abs(min(v.co.z for v in floor.data.vertices)-R['floor_bottom_m'])<1e-6
assert all(abs(v.co.z-R['floor_top_m'])<1e-6 or abs(v.co.z-R['floor_bottom_m'])<1e-6 for v in floor.data.vertices)
tops=[p for p in floor.data.polygons if p.normal.z>.99]
assert tops and all(all(abs(floor.data.vertices[i].co.z-R['floor_top_m'])<1e-6 for i in p.vertices) for p in tops)
...
# Independently measure the final saved black roof, including modifiers.
deps=bpy.context.evaluated_depsgraph_get();black_highest=-1e9
for o in bpy.data.collections['05 Roof - estimated'].objects:
 if o.type!='MESH' or not any(m and m.name=='Roof | new black coated metal' for m in o.data.materials):continue
 e=o.evaluated_get(deps);mesh=e.to_mesh();black_highest=max(black_highest,max((e.matrix_world@v.co).z for v in mesh.vertices));e.to_mesh_clear()
top=max(v.co.z for v in floor.data.vertices);bottom=min(v.co.z for v in floor.data.vertices)
assert abs(top-black_highest-.10)<1e-6,(top,black_highest)
assert top-bottom>=.30-1e-6 and abs(top-bottom-.30)<1e-6
report={'concept_file_reopened':True,'original_house_file_unchanged':True,'flat_floor_top_m':top,'floor_bottom_m':bottom,'floor_thickness_m':top-bottom,'black_roof_highest_m':black_highest,'clearance_above_black_roof_m':top-black_highest,'floor_area_m2':area,'closed_floor_solid':True,'red_roof_geometry_removed':True,'black_roof_and_retained_trim_meshes_identical':len(R['protected_black_roof_objects']),'black_roof_slopes_percent':slopes,'unrelated_meshes_identical':len(R['unchanged_mesh_signatures']),'wall_xy_and_openings_preserved':True,'hidden_wall_top_clearance_m':.001,'wood_material_verified':True,'mansard_added':False,'scene_count':17}
